...
int main(int argc, char **argv) {
    ...
    // Written Halide schedules below
    // HALIDE SCHEDULER 1
    Var x_outer, y_outer, x_inner, y_inner, tile_index;
    dilated_conv.tile(x, y, x_outer, y_outer, x_inner, y_inner, 4, 4);
    dilated_conv.fuse(x_outer, y_outer, tile_index);
    dilated_conv.vectorize(x_inner, 4);
    dilated_conv.parallel(tile_index);
    dilated_conv.compute_root();
    dilated_conv.update().tile(x, y, x_outer, y_outer, x_inner, y_inner, 4, 4)
    .fuse(x_outer, y_outer, tile_index)
    .vectorize(x_inner, 4)
    .parallel(tile_index);
    
    // HALIDE SCHEDULER 2 (not working)
    // Var x_outer, y_outer, x_inner, y_inner, tile_index, co_outer, co_inner;
    // dilated_conv.tile(x, y, x_outer, y_outer, x_inner, y_inner, 8, 8);
    // dilated_conv.split(c, co_outer, co_inner, 16);
    // dilated_conv.fuse(x_outer, y_outer, tile_index);
    // dilated_conv.fuse(tile_index, co_outer, tile_index);
    // dilated_conv.vectorize(x_inner, 4);
    // dilated_conv.vectorize(co_inner, 4);
    // dilated_conv.parallel(tile_index);
    // dilated_conv.compute_root();
    // dilated_conv.update().reorder(r.x, r.y, r.z, x_inner, y_inner, co_inner, x_outer, y_outer, co_outer);
    // dilated_conv.update().vectorize(x_inner, 4);
    // dilated_conv.update().vectorize(co_inner, 4);
    // dilated_conv.update().parallel(tile_index);
